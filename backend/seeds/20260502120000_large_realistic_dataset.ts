      ...
      orgSeed.doctors.forEach((doctor, doctorIndex) => {
        ...
        for (let slotIndex = 0; slotIndex < plannedSlots; slotIndex += 1) {
          ...
          appointmentRows.push({
            ...
            cancellation_reason:
              status === "CANCELLED"
                ? pick(cancellationReasons, appointmentCount)
                : null,
          ...
          if (dayOffset >= 0) {
            reminderRows.push({
              ...
              sent_at: null,
              status:
                dayOffset === 0 && appointmentCount % 11 === 0
                  ? "FAILED"
                  : "PENDING",
              attempt_count:
                dayOffset === 0 && appointmentCount % 11 === 0 ? 2 : 0,
              last_error:
                dayOffset === 0 && appointmentCount % 11 === 0
                  ? "Privremena greska dostave"
                  : null,
            });
